        ...
        <div className="space-y-4">
          ...
          <div>
            <button
              ...
              disabled={!file || (status?.status === 'uploading' || status?.status === 'processing') || !linkedinUrl || !!linkedinUrlError}
              className={`inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white disabled:opacity-50 disabled:cursor-not-allowed ${
                !file || (status?.status === 'uploading' || status?.status === 'processing') || !linkedinUrl || !!linkedinUrlError
